...
@Slf4j
@AllArgsConstructor
public class UserPlaceOrderIntegrationEventHandler implements IIntegrationEventHandler<UserPlaceOrderIntegrationEvent> {

    private final IOrderingService orderingService;

    @Override
    public void handle(UserPlaceOrderIntegrationEvent event) {
        log.info("🛒 UserPlaceOrderIntegrationEvent received for UserId: {}", event.getUserId());
